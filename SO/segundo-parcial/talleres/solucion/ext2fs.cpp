...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	unsigned int blockgroup_number = blockgroup_for_inode(inode_number);	   //grupo para inodo
...
// }
	...
	unsigned int cant_inodes_per_block = block_size / _superblock->inode_size	//cant_inodos

	//Busco el numero de bloque dentro de la tabla de inodos
	unsigned int block_number = offset_for_inode / cant_inodes_per_block;	//esto me da en qué bloque queda
	read_block(table_start+block_number, block_buf);						//leo
	
	//Busco cual inodo es dentro del bloque
	unsigned int block_offset = offset_for_inode % cant_inodes_per_block;	//esto me da el offset YA DENTRO del bloque

	//Dentro del bloque, copio el contenido del inodo que necesito a un struct inodo
	for(int i=0;i<sizeof(Ext2FSInode);i++){
		Ext2FSInode_buf[i] = block_buf[(block_offset *_superblock->inode_size) + i]; //copio a un "recipiente" de inodo
	}
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	// Ejercicio 1
	if(block_number < DIRECT_ENTRY_LIMIT){ //12
    // Directo
    ...
    // Primera indirección
		if(block_number < FIRST_INDIRECT_LIMIT){ //256+12
			
			// Me traigo la direccion de disco de la tabla de punteros y la leo en block_buf
			unsigned int block_address = inode->block[FIRST_INDIRECT_BLOCK_ENTRY]; //entrada 13 (12)
    ...
    // Segunda indirección
			...
			unsigned int block_address = inode->block[SECOND_INDIRECT_BLOCK_ENTRY]; //entrada 14 (13)
			read_block(block_address, block_buf);
